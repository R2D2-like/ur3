...
from lfd_proposed_five_layers import LfDProposed
...
class RolloutProposed:
    def __init__(self) -> None:
        ...
        model_weights_path = '/root/Research_Internship_at_GVlab/ablation/real/model/proposed/proposed_model4_five_layers.pth'
    ...
    def rollout(self):
        ...
        save_dir = self.base_save_dir + 'proposed/result/five_layers/'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        save_dir = self.base_save_dir + 'proposed/predicted/five_layers/'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        save_path = save_dir + self.sponge + '_' + self.height +'.npz'
        # save_path = save_dir + self.sponge + '_' + self.height +'_kp{}.npz'.format(self.kp)
