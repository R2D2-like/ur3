...
class RolloutProposed:
    def __init__(self) -> None:
        ...
        self.base_save_dir = '/root/Research_Internship_at_GVlab/ablation/real/rollout/data/'
        ...
        model_weights_path = '/root/Research_Internship_at_GVlab/ablation/real/model/proposed/proposed_model4_window10.pth'
    ...
    def rollout(self):
        ...
        for i in range(1500, 2000, 20):
            ...
            ft_history = np.array(self.arm.get_wrench_history(hist_size=200))[::20] # (5, 6)
        ...
        save_dir = self.base_save_dir + 'proposed/result/window10/'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        save_dir = self.base_save_dir + 'proposed/predicted/window10/'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        save_path = save_dir + self.sponge + '_' + self.height +'.npz'
        # save_path = save_dir + self.sponge + '_' + self.height +'_kp{}.npz'.format(self.kp)
