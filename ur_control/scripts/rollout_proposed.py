...
class Rollout:
    def __init__(self) -> None:
        ...
        self.eef_position_history = collections.deque(maxlen=100)

        self.save_dir = '/root/Research_Internship_at_GVlab/'
        self.save_name = input('Enter the name of the save file for inference results: ')

        exploratory_data_name = input('Enter the name of the exploratory data file: ')
        self.vae_inputs = np.load(self.save_dir + 'real/rollout/data/exploratory/'+ exploratory_data_name + '.npy') # normalized

        model_weights_path = self.save_dir + 'real/model/propose.pth'
    ...
    def eef_pose_callback(self, msg):
        ...
        self.eef_position_history.append(current_pos)
    ...
    def rollout(self):
        ...
        while (rospy.Time.now() - start_time).to_sec() < 20:
            ...
            eef_position_history = self.position2input(self.eef_position_history) # (100, 3)
